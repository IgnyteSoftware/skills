...
Because it's one file with nothing to install, you can hand it to a non-developer such as a designer, PM, or domain expert and let them feel the model for themselves. So it speaks their language, not the code's.
...
This is the wrong branch if the question is "what should this look like." Use [UI.md](UI.md).
...
4. **Guided walkthroughs**. Create a set of **scenarios**, one per tab. Each tab holds a short plain-language description of the scenario, including the situation it sets up and what to watch for. Underneath it, show the ordered **buttons to press** for that scenario. Each step is a real button: clicking it performs that action and moves to the next step. Starting a walkthrough resets to a known initial state so the scenario runs the same way every time.

Choose scenarios that demonstrate the awkward cases: the happy path, a tricky edge case, and an attempt at something that should be illegal. Focus on cases that are hard to reason about on paper.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell rides along to the throwaway branch that keeps the prototype as a primary source. As one self-contained file, it stays trivially re-runnable there.
